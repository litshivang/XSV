...
from modules.extractor_llm import LLMTravelExtractor
...
class OptimizedTravelAgentProcessor:
    ...
    def __init__(self):
        ...
        self.llm_extractor = LLMTravelExtractor()
        self.inquiry_classifier = OptimizedInquiryClassifier()
        self.excel_generator = OptimizedExcelGenerator()
        logger.info("Optimized Travel Agent Processor initialized with LLM and rule-based modules")
    ...
    def process_inquiry(self, email_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Step 3: Comprehensive Field Extraction - Try LLM first, fallback to rule-based
            try:
                # Use LLM extraction for better field coverage
                llm_data = self.llm_extractor.extract_trip_inquiry(body, subject)
                if llm_data.get('extraction_confidence', 0) > 0.5:
                    extracted_fields = llm_data
                    logger.info("Using LLM extraction results")
                else:
                    raise Exception("LLM extraction confidence too low")
