...
Provides centralized logging configuration with separate log files for each module
"""
...
def setup_logger(name: str, level: str = None) -> logging.Logger:
    """
    Setup logger with file and console handlers, using a separate log file for each module
    
    Args:
        name: Logger name (e.g., 'gmail', 'outlook')
    ...
    if logger.handlers:
        logger.debug(f"Returning existing logger for {name}")
        return logger
    ...
    try:
        ...
        # Use module-specific log file (e.g., logs/gmail.log)
        log_file = config.LOGS_DIR / f"{name}.log"
        logger.debug(f"Setting up file handler for {log_file}")
        ...
        logger.warning(f"Could not setup file logging for {name}: {str(e)}")
    
    logger.info(f"Logger initialized for {name} with log file: {log_file}")
    return logger
